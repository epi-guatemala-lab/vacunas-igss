...
export default function SuccessScreen({ onNewForm, isOffline, registroId, trabajadorNombre, departamentoIGSS }) {
  const handleCopy = useCallback(() => {
    const text = [
      '════════════════════════════════',
      'CONSTANCIA DE REGISTRO',
      'Censo de Vacunación — IGSS',
      '════════════════════════════════',
      '',
      `No. Registro: ${registroId}`,
      `Trabajador: ${trabajadorNombre || 'N/A'}`,
      `Departamento: ${departamentoIGSS || 'N/A'}`,
  ...
  }, [registroId, trabajadorNombre, departamentoIGSS])
      ...
      <div className="max-w-sm mx-auto rounded-2xl overflow-hidden shadow-igss-lg border border-igss-200">
        ...
        <div className="bg-gradient-to-r from-igss-800 to-igss-700 px-5 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-igss-400">Censo de Vacunación — Sarampión SPR/SR</p>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {trabajadorNombre && (
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Trabajador</p>
                  <p className="text-sm text-gray-800 font-semibold">{trabajadorNombre}</p>
                </div>
              </div>
            )}
            {departamentoIGSS && (
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-800 font-semibold">{departamentoIGSS}</p>
                </div>
              </div>
            )}
            <div className="flex items-start gap-2">
